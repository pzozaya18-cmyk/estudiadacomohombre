...
export default function App() {
  ...
  return (
    <div className="relative min-h-screen selection:bg-white selection:text-primary">
      ...
      {/* Navigation */}
      <nav className="fixed top-0 left-0 right-0 z-50 p-4 sm:p-6 flex justify-between items-start gap-4 pointer-events-none">
        <div className="text-white font-bold text-[clamp(0.82rem,2.4vw,1.25rem)] uppercase tracking-[0.18em] pointer-events-auto">
          {t.nav.logo}
        </div>
        <button 
          onClick={() => setLang(lang === 'es' ? 'en' : 'es')}
          className="text-white font-semibold text-[clamp(0.82rem,2.4vw,1.125rem)] hover:opacity-70 transition-opacity pointer-events-auto uppercase"
        >
          {lang === 'es' ? 'es | en' : 'en | es'}
        </button>
      </nav>
      ...
      <main className="relative z-10">
        {t.screens.map((screen, index) => {
          ...
          if (screen.layout === 'final') {
            return <FinalSection key={screen.id} screen={screen} />;
          }

          if (screen.id === 4) {
            return <EightyPercentSection key={screen.id} screen={screen} />;
          }
...
const BeatsSection: React.FC<{ screen: any }> = ({ screen }) => {
  ...
  const getBeatConfig = (index: number) => {
    ...
    if (screen.id === 14 || screen.id === 15 || screen.id === 16) {
      return { start: 0.1, end: 0.9, stay: true, yOffset: 0, xOffset: 0 };
    }

    if (screen.id === 17) {
      if (index === 0) {
        return { start: 0.0, end: 1.0, stay: true, yOffset: -190, xOffset: '-8%' };
      }
      return { start: 0.48, end: 1.0, stay: true, yOffset: 78, xOffset: '0%' };
    }
  ...
  return (
    <section ref={containerRef} className={`relative ${screen.id === 13 || screen.id === 18 ? 'h-[600vh]' : screen.id === 10 || screen.id === 11 || screen.id === 12 || screen.id === 17 ? 'h-[320vh]' : 'h-[240vh]'}`}>
...
const BeatItem: React.FC<{ 
  ...
  const opacityEnd = stay ? 0.98 : end;
  const opacity = useTransform(
    progress, 
    stay ? [start, start + 0.06, opacityEnd - 0.08, opacityEnd] : [actualStart, actualHoldStart, actualHoldEnd, actualEnd], 
    [0, 1, 1, 0]
  );
  ...
  const entryY = useTransform(
    progress, 
    stay ? [start, start + 0.06] : [actualStart, actualHoldStart - (end - start) * 0.05, actualHoldStart], 
    stay ? [40, 0] : [40, -4, 0]
  );
  
  // Shared Exit behavior for the group (drift up together at the very end)
  const groupExitY = useTransform(progress, [0.92, 0.98], [0, -40]);
  ...
  const scale = useTransform(progress, [start, start + 0.08, opacityEnd - 0.08, opacityEnd], [isCard ? 0.95 : 0.98, 1, 1, 1.02]);
  ...
  return (
    <motion.div 
      style={{ 
        ...
        y,
        x,
        display: useTransform(progress, (p: number) => (p >= start - 0.02 && (stay ? p < 0.99 : p < end + 0.02) ? 'flex' : 'none')) 
      }}
    ...
    >
      ...
      `}>
        {beat.text.split('\n').map((line: string, li: number) => {
          ...
          return (
            <div key={li} className={`flex flex-wrap justify-center gap-y-[0.1em] ${screenId === 14 || screenId === 15 || screenId === 16 || screenId === 17 ? 'gap-x-[0.25em] md:gap-x-[0.55em]' : 'gap-x-[0.18em] md:gap-x-[0.3em]'} ${screenId === 14 || screenId === 16 ? 'large-display-flex' : ''}`}>
    ...
    <motion.span
      ...
      className={`
        ${isFinalHashtagWord(screenId, index)
          ? 'text-[clamp(3.2rem,13vw,10rem)] font-black text-white/40'
          : isFinalMainWord(screenId, index)
            ? 'text-[clamp(1.65rem,5.8vw,4.8rem)] font-black text-white drop-shadow-[0_0_34px_rgba(255,255,255,0.18)]'
...
const ComparisonSection: React.FC<{ screen: any; graphic?: any }> = ({ screen, graphic }) => {
  ...
  const revealLines = [
    { text: screen.top, kind: 'setup', range: [0.0, 0.12] as [number, number] },
    { text: screen.middle, kind: 'number', range: [0.1, 0.25] as [number, number] },
...
const ComparisonRevealLine: React.FC<{
  ...
  range: [number, number];
  sectionProgress: number;
}> = ({ text, kind, range, sectionProgress }) => {
  const [start, end] = range;
  const isRendered = sectionProgress >= start;
  ...
  const opacity = revealProgress >= 1 ? 1 : easedProgress;
  const y = revealProgress >= 1 ? 0 : 55 * (1 - easedProgress);

  const words = text.split(/\s+/).filter(Boolean);
  const lineClass =
    kind === 'number'
      ? 'text-[clamp(4.4rem,15.5vw,15rem)] leading-none my-[clamp(0.15rem,0.8vh,0.7rem)]'
      : kind === 'setup'
        ? 'text-[clamp(1.15rem,3vw,2.55rem)] leading-[1.06]'
        : 'text-[clamp(1.75rem,5.2vw,4.1rem)] leading-[1.04]';
...
const WordReveal: React.FC<{ text: string; progress: any }> = ({ text, progress }) => {
  ...
  return (
    <div className="flex flex-wrap justify-center gap-x-[0.15em] gap-y-[clamp(0.65rem,2vw,2rem)]">
      {words.map((word, i) => (
        <WordRevealItem 
          key={i} 
          word={word} 
...
const WordRevealItem: React.FC<{ 
  word: string; 
  index: number; 
  total: number; 
  progress: any 
}> = ({ word, index, total, progress }) => {
  // Start reveal at 10% section progress, end by 70%
  ...
  return (
    <motion.span
      style={{ opacity, y, scale }}
      className="text-[clamp(2.2rem,9vw,7.5rem)] font-black leading-[0.95] md:leading-[0.88] tracking-normal inline-block text-white"
    >
      {word}
    </motion.span>
  );
...
const SimpleSection: React.FC<{ screen: any; graphic?: any; isLast: boolean }> = ({ screen, graphic, isLast }) => {
  ...
  // Section level fade out
  const opacity = useTransform(scrollYProgress, [0.7, 0.9], [1, 0]);
  const scale = useTransform(scrollYProgress, [0.7, 0.9], [1, 0.95]);

  return (
    <section ref={ref} className="min-h-dvh flex items-center justify-center sticky-safe relative overflow-hidden">
      ...
      >
        <div className="uppercase tracking-normal whitespace-pre-line">
          {screen.id === 3 ? (
            <WordReveal text={screen.text || ""} progress={scrollYProgress} />
          ) : (
      ...
      return (
        <section ref={containerRef} className="relative h-[360vh]">
          <div className="sticky top-0 h-dvh w-full flex items-center justify-center overflow-visible sticky-safe">
            ...
            <div className="absolute inset-x-0 top-0 h-full z-20 flex items-center justify-center pointer-events-none overflow-visible">
              <div className="relative h-full w-full max-w-[min(88vw,760px)]">
                <MedicalTextBlock
                  text={screen.scrolls[0]}
                  progress={scrollYProgress}
                  visibleRange={[0.05, 0.5]}
                  enterRange={[0.05, 0.24]}
                  holdRange={[0.24, 0.34]}
                  exitRange={[0.34, 0.5]}
                />
                <MedicalTextBlock
                  text={screen.scrolls[1]}
                  progress={scrollYProgress}
                  visibleRange={[0.52, 0.95]}
                  enterRange={[0.52, 0.71]}
                  holdRange={[0.71, 0.8]}
                  exitRange={[0.8, 0.95]}
                />
              </div>
...
const ViolenceSection: React.FC<{
  ...
  return (
    <section ref={containerRef} className="relative h-[390vh]">
      <div className="sticky top-0 h-dvh w-full flex items-center justify-center sticky-safe overflow-visible">
        <div className="violence-stage text-center font-black uppercase">
          {lines.map((line: string, index: number) => (
            <ViolenceStackLine
              key={`${line}-${index}`}
              line={line}
...
const ViolenceStackLine: React.FC<{
  ...
  const [start, end] = revealRanges[index] || [0, 0];
  const revealProgress = index < 4 ? 1 : Math.min(Math.max((sectionProgress - start) / (end - start), 0), 1);
  const easedProgress = easeInOut(revealProgress);
  const y = index < 4 ? 0 : 92 * (1 - easedProgress);
  const opacity = index < 4 ? 1 : easedProgress;
  const lineClass = index === 0 || index === 2
    ? 'violence-line-small'
    : index >= 4 || index === 1 || index === 3
      ? 'violence-line-main'
      : '';
  const slotClass = index === 0 || index === 2
    ? 'violence-slot-small'
    : index >= 4 || index === 1 || index === 3
      ? 'violence-slot-main'
      : '';
...
const FinalSection: React.FC<{ screen: any }> = ({ screen }) => {
  const ref = useRef(null);
        ...
        {screen.subtext && (
          <div className="mt-6 max-w-3xl">
            {screen.subtext.split('\n').map((line: string, i: number) => (
              <p key={i} className="text-[clamp(1rem,2.3vw,1.5rem)] font-medium text-white/70 leading-relaxed uppercase tracking-normal">
                {line}
              </p>
            ))}
          </div>
